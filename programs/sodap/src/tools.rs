...
pub mod sodap {
    ...
    pub fn register_product(
        ...
        if tokenized_type == TokenizedType::SplToken {
            // The product mint must have its mint authority set to the PDA:
            // Pubkey::find_program_address([b"product_mint", product_uuid], program_id)
            // The product_mint_authority account here is this PDA.
            ...
            let authority_seeds = &[b"product_mint", product_uuid.as_ref(), &[bump]];
    ...
    pub fn mint_loyalty_tokens(ctx: Context<MintLoyaltyTokens>, amount: u64) -> Result<()> {
        ...
        // Use the correct PDA seeds for mint_authority
        ...
        initialize_mint(cpi_ctx, 0, ctx.accounts.mint_authority.key, None)?;
    ...
    pub fn initialize_loyalty_mint(ctx: Context<InitializeLoyaltyMint>) -> Result<()> {
        // Only the store owner or platform admin can initialize
        let store = &ctx.accounts.store;
        ...
        let seeds: [&[u8]; 3] = [
            b"loyalty_mint",
            store_key.as_ref(),
            &[ctx.bumps.loyalty_mint_authority],
        ];
        let signer_seeds: &[&[u8]] = &seeds;
        let binding = [signer_seeds];
        let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts, &binding);
        initialize_mint(cpi_ctx, 0, ctx.accounts.loyalty_mint_authority.key, None)?;
...
fn has_role(store: &Store, user: &Pubkey, role: AdminRoleType) -> bool {
    store
        .admin_roles
        .iter()
